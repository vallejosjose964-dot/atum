...
const globalMetric = document.getElementById('globalMetric');
const btnRAR = document.getElementById('btnRAR');
...
function computeAccelerations(rows){
  // Devuelve puntos (log g_bar, log g_obs) en unidades SI.
  const pts = [];
  for(const r of rows){
    const Rkpc = Number(r.R_kpc);
    if(!Number.isFinite(Rkpc) || Rkpc <= 0) continue;

    const Rm = Rkpc * 3.085677581e19; // kpc -> m
    const Vobs = Number(r.Vobs) * 1000;
    const Vgas = Number(r.Vgas||0);
    const Vdisk= Number(r.Vdisk||0);
    const Vbul = Number(r.Vbul||0);

    const Vbar = Math.sqrt(Math.max(0, Vgas*Vgas + Vdisk*Vdisk + Vbul*Vbul)) * 1000;

    const g_obs = (Vobs*Vobs)/Rm;
    const g_bar = (Vbar*Vbar)/Rm;

    if(g_obs>0 && g_bar>0 && Number.isFinite(g_obs) && Number.isFinite(g_bar)){
      pts.push({ x: Math.log(g_bar)/Math.LN10, y: Math.log(g_obs)/Math.LN10 });
    }
  }
  return pts;
}

function renderRAR(points, label){
  clearCanvas();
  if(!points || points.length < 2){
    log('RAR: no hay puntos suficientes (revisar columnas Vgas/Vdisk/Vbul).');
    return;
  }
  const b = boundsFor(points);
  drawAxes(b, 'log10(g_bar)  [m/s²]', 'log10(g_obs)  [m/s²]');
  plotSeries(points, b, `RAR: ${label}`, 'rgba(110,231,255,0.95)');
}
...
function setMacroStructureFromBackend(resp){
...
function setMacroStructureLocal(parsed){
  // Definición simple y reproducible:
  // centro = primer punto; borde = último punto; Vmax = max(Vobs)
  ...
  const vcenter = Number(first.Vobs);
  const rcenter = Number(first.R_kpc);
  const vedge   = Number(last.Vobs);
  const redge   = Number(last.R_kpc);

  let vmax = -Infinity;
  for(const r of rows){
    const v = Number(r.Vobs);
    if(Number.isFinite(v)) vmax = Math.max(vmax, v);
  }

  // slope_center aproximada con los dos primeros puntos
  const r2 = Number(rows[1].R_kpc);
  const v2 = Number(rows[1].Vobs);
  let slope = NaN;
  if(Number.isFinite(rcenter) && Number.isFinite(r2) && r2 !== rcenter && Number.isFinite(vcenter) && Number.isFinite(v2)){
    slope = (v2 - vcenter) / (r2 - rcenter);
  }

  const fmt = (x, n=2) => (Number.isFinite(Number(x)) ? Number(x).toFixed(n) : '—');
  macroVCenter.textContent = fmt(vcenter, 2);
  macroVEdge.textContent   = fmt(vedge, 2);
  macroVMax.textContent    = fmt(vmax, 2);
  macroRCenter.textContent = fmt(rcenter, 3);
  macroREdge.textContent   = fmt(redge, 3);
  macroSlope.textContent   = fmt(slope, 3);
}
...
async function computeSelected(){
  ...
  try{
    ...
    const used = setMacroStructureFromBackend(resp);
    if(!used) setMacroStructureLocal(parsed);
...
async function globalRMS(){
  ...
  try{
    ...
    if(globalMetric) globalMetric.textContent = String(agg ?? "—");
...
function resetAll(){
  ...
  setMacroStructureLocal({rows:[]});
  if(globalMetric) globalMetric.textContent='—';
  log('RESET');
}
...
btnRAR && btnRAR.addEventListener('click', async ()=>{
  try{
    const {name, text} = await readSelected();
    const parsed = parseCurveFile(name, text);
    const label = name.split('/').pop().replace(/\.(dat|csv|txt)$/i,'');
    const pts = computeAccelerations(parsed.rows);
    renderRAR(pts, label);
    log(`RAR ok: ${label} (n=${pts.length})`);
  }catch(e){
    log(`ERROR RAR: ${e}`);
  }
});
