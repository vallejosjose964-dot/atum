...
async function loadZipFromUrl(url){
  ...
  if(!r.ok) throw new Error(`No pude bajar ZIP (${r.status})`);
  ...
  if(!zipFiles.length){
    fileSelect.disabled = true;
    fileSelect.innerHTML = '<option value="">(no hay CSV/TXT en el ZIP)</option>';
    log('ZIP bajado pero no encontré CSV/TXT adentro.');
    return;
  }
...
(function boot(){
  ...
  // Auto-try repo ZIP (no rompe si no existe)
  (async ()=>{
    try{
      const url = (repoZipUrl && repoZipUrl.value ? repoZipUrl.value.trim() : './Rotmod_LTG.zip');
      await loadZipFromUrl(url);
      log('Auto-load repo ZIP OK.');
    }catch(e){
      log('Auto-load repo ZIP: no disponible (ok).');
    }
  })();
